...
#	docker-compose -f ~/Projects/traefik.yml up -d
	@echo ""
	@echo ""
	@echo "${green} Your app is now available at: http://$(PROJECT_BASE_URL):$(PROJECT_PORT) ${NC}"
...
start:
	...
	@echo ""
	@echo ""
	## @echo "Your app is now available at: http://$(PROJECT_BASE_URL):{PROJECT_PORT}"
	@echo "${green} Your app is now available at: http://$(PROJECT_BASE_URL):$(PROJECT_PORT) ${NC}"
...
db_export:
	docker-compose exec mariadb sh -c 'exec mysqldump -u root -p"$(DB_ROOT_PASSWORD)" $(DB_NAME)' > ./mariadb-init/db_backup.sql

## db_import :	Import the new database from the mariadb-init directory.
db_import:
	docker-compose exec mariadb sh -c 'exec mysql -u root -p"$(DB_ROOT_PASSWORD)" $(DB_NAME) < /docker-entrypoint-initdb.d/db_backup.sql'
...
## To get the command arguments from user -  $(filter-out $@,$(MAKECMDGOALS))
build:
	...
	docker-compose up -d --remove-orphans
#	docker-compose -f ~/Projects/traefik.yml up -d
	@echo "${green_bg} Step 2${NC}${green} Checking hosts file ${NC}"
	@echo ""
...
rebuild:
	...
	@echo ""
	@echo "Please place a database backup (.sql) file in the mariadb_init directory of the project and provide the file name (along with the sql extension). Enter 'skip' to skip this step."
	@read -p "Enter Database Backup File name (enter to skip):" db_file_name
	@echo ""

 	ifndef $(db_file_name)
		@echo "No input entered. Skipping db import.."
 	else
	@docker-compose exec mariadb sh -c 'exec mysql -u root -p"$(DB_ROOT_PASSWORD)" $(DB_NAME) < /docker-entrypoint-initdb.d/$(db_file_name)'
 	endif
	@echo ""
	@echo ""
